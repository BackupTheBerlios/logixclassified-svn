...
#
#################################################################################################
if(!strpos($_SERVER['PHP_SELF'],'members_overview.php') === false)
{
...
if ($show_members_overview && (!($sales_option && $sales_members>1) || $is_sales_user)) {
    ...
    $sqlsearch=" AND username LIKE '%$value%'";
    }

    $amount = mysql_query("SELECT count(id) FROM ".$prefix."userdata WHERE language<>'xc' AND language<>'xd'".$sqlsearch);
    ...
    if ($pages) {                                       // print only when pages > 0
        ...
        echo "[<a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=$orders&sort=$sort&offset=$noffset&poffset=$npoffset\"><</a>] ";
    }
        ...
        $actual = $i + 1;
            if ($actual==$actpage) {
        echo "(<a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=$orders&sort=$sort&offset=$noffset&poffset=$npoffset\">$actual</a>) ";
            } else {
        echo "[<a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=$orders&sort=$sort&offset=$noffset&poffset=$npoffset\">$actual</a>] ";
        }
    }
    ...
    $result = mysql_query("select * FROM ".$prefix."userdata WHERE language<>'xc' AND language<>'xd'".$sqlsearch." ORDER BY $orders $sort LIMIT $offset, $perpage") or die("Database Query Error");

    echo " <table align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\" width=\"100%\">\n";
    echo "   <tr>\n";
    echo "    <td class=\"gbheader\" align=\"left\">\n";
    echo "      <div class=\"smallleft\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=username&sort=asc&offset=$offset&poffset=$poffset\">$memb_detuser</a></div>";
    echo "    </td>\n";
    echo "    <td class=\"gbheader\" width=\"30\">\n";
    echo "      <div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=ads&sort=desc&offset=$offset&poffset=$poffset\">$memb_detads</a></div>";
    echo "    </td>\n";
    echo "    <td class=\"gbheader\" width=\"30\">\n";
    echo "      <div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=votes&sort=desc&offset=$offset&poffset=$poffset\">$memb_detvot</a></div>";
    echo "    </td>\n";
    echo "    <td class=\"gbheader\" width=\"30\">\n";
    echo "      <div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=email&sort=asc&offset=$offset&poffset=$poffset\">$memb_detmail</a></div>";
    echo "    </td>\n";
    echo "    <td class=\"gbheader\" width=\"30\">\n";
    echo "      <div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=icq&sort=desc&offset=$offset&poffset=$poffset\">$memb_deticq</a></div>";
    echo "    </td>\n";
    echo "    <td class=\"gbheader\" width=\"30\">\n";
    echo "      <div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=homepage&sort=desc&offset=$offset&poffset=$poffset\">$memb_deturl</a></div>";
    echo "    </td>\n";
    echo "    <td class=\"gbheader\" width=\"30\">\n";
    echo "      <div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?action=$action&value=$value&orders=picture&sort=desc&offset=$offset&poffset=$poffset\">$memb_detpic</a></div>";
    echo "    </td>\n";
    ...
    if ($show_members_details) {
            ...
            echo "<div class=\"smallcenter\"><a href=\"$_SERVER[PHP_SELF]?choice=details&uid=$db[id]&uname=$db[username]\">$memb_detdetails</a>\n";
        }
    }
