...
#
#################################################################################################
if(!strpos($_SERVER['PHP_SELF'],'int_phpBB-V1.4.x.php') === false)
{
...
mysql_select_db ($forum_db_name,$forum_db_connect_id) or die(mysql_error());
    ...
    if (!$intresult) {
    // Get the next ID !!!!
    $query=mysql_query("SELECT user_id, username FROM $USERS_TABLE
        WHERE user_id <> '-1' ORDER BY user_id DESC LIMIT 1",$forum_db_connect_id) or die(mysql_error());
    $dbt=mysql_fetch_array($query);
        $id=$dbt[user_id]+1;

    // Add new Member
        $md5password=md5($password);
    mysql_query("insert into $USERS_TABLE (user_id, username, user_password, user_email, user_regdate, user_theme)
        values ('$id', '$username', '$md5password', '$email', '$timestamp','2')",$forum_db_connect_id) or die(mysql_error());
    }
