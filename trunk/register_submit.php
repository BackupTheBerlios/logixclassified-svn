...
if ($homepage=="http://") {$homepage="";}
    ...
    {
        $fieldname=$db['name'];
#KLUDGE: member_submit.php - brian - In order to prevent undefined index errors, we do the below, however it *breaks* the checking for required fields
#Initialize required error var
$requirederror="";
        if (in_array($fieldname,$_POST))
        {
    ...
    if (empty($register)) {
        ...
        if ($result > 0) {
            ...
            #TODO: What if they do *NOT* have a forum installed? We need to check this stuff during setup, or admin panel deal
            #THIS IS A HUGE HOLE- even if config is included, it's possible to get around that with a little creativity
