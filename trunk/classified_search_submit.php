...
if ($in[searchmode]=="simple") {
    ...
    if ($in[adid]) {
        ...
        $result = mysql_query("SELECT * FROM ".$prefix."ads WHERE id=$in[adid]");
        if ($result) {
        ...
        if ($db && $catid && $subcatid) {
    	    header(headerstr("classified.php?catid=$catid&subcatid=$subcatid&adid=$in[adid]"));
	    exit;
        }
    }
	...
	if ($in[picture]) {
	    $sqlquerystr.=" AND (picture1!='' OR picture2!='' OR picture3!='')";
	}
	if ($in[attachment]) {
