<?
if(!strpos($_SERVER['PHP_SELF'],'classified_results.php') === false)
{
...
if (!$adid && $sqlquery) {
    ...
    $sql = "SELECT count(*) FROM ".$prefix."ads".stripslashes($sqlquery.$sql2.$sqlquery3);
    ...
    if ($npoffset>$maxpoffset) {$npoffset = $maxpoffset;}
        echo "[<a href=\"classified.php?sqlquery=$nsqlquery&sqlquery2=$nsqlquery2&sqlquery3=$nsqlquery3&offset=$noffset&poffset=$npoffset\" onmouseover=\"window.status='$nav_prev'; return true;\" onmouseout=\"window.status=''; return true;\"><</a>]\n";
      }
    ...
    $actual = $i + 1;
        if ($actual==$actpage) {
        echo "(<a href=\"classified.php?sqlquery=$nsqlquery&sqlquery2=$nsqlquery2&sqlquery3=$nsqlquery3&offset=$noffset&poffset=$npoffset\" onmouseover=\"window.status='$nav_actpage'; return true;\" onmouseout=\"window.status=''; return true;\">$actual</a>)\n";
        } else {
        echo "[<a href=\"classified.php?sqlquery=$nsqlquery&sqlquery2=$nsqlquery2&sqlquery3=$nsqlquery3&offset=$noffset&poffset=$npoffset\" onmouseover=\"window.status='$nav_gopage'; return true;\" onmouseout=\"window.status=''; return true;\">$actual</a>]\n";
        }
      }
    ...
    if ($npoffset<0) {$npoffset = 0;}
    if ($npoffset>$maxpoffset) {$npoffset = $maxpoffset;}
        echo "[<a href=\"classified.php?sqlquery=$nsqlquery&sqlquery2=$nsqlquery2&sqlquery3=$nsqlquery3&offset=$noffset&poffset=$npoffset\" onmouseover=\"window.status='$nav_next'; return true;\" onmouseout=\"window.status=''; return true;\">></a>]\n";
      }
    }
    ...
    $sqlquery = "SELECT * FROM ".$prefix."ads".stripslashes($sqlquery.$sql2.$sqlquery3)." LIMIT $offset, $perpage";
