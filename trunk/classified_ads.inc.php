...
#
#################################################################################################
if(!strpos($_SERVER['PHP_SELF'],'classified_ads.inc.php') === false)
{
  die("YOU MAY NOT ACCESS THIS FILE DIRECTLY");
        ...
        if ($db[picture1] || $db[picture2] || $db[picture3]) {

        if (!$db[picture1] && $db[picture2]) {
            $db[picture1]=$db[picture2];
            $db[_picture1]=$db[_picture2];
        } elseif (!$db[picture1] && !$db[picture2] && $db[picture3]) {
            $db[picture1]=$db[picture3];
            $db[_picture1]=$db[_picture3];
        }

            if (!$pic_database && $db[picture1] && is_file("$pic_path/$db[picture1]") && $pic_prelowres) {
            // claculate thumbnail-size
            if (is_file("$pic_path/$db[_picture1]")) {
            $previewpic="$pic_path/$db[_picture1]";
            } else {
            $previewpic="$pic_path/$db[picture1]";
            }
        ...
        } elseif ($db[picture1] && $pic_prelowres) {
            // claculate thumbnail-size
            $result4 = mysql_query("SELECT * FROM ".$prefix."pictures WHERE picture_name='$db[_picture1]'") or died("Can NOT find the Picture");
            $dbp = mysql_fetch_array($result4);
            if (!$dbp) {
            $result4 = mysql_query("SELECT * FROM ".$prefix."pictures WHERE picture_name='$db[picture1]'") or died("Can NOT find the Picture");
            $dbp = mysql_fetch_array($result4);
            }
            $picsize=explode("x",$pic_prelowres);
